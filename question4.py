...
server_overload_log = []
server_network_failure = []
server_network_failure_log = []
...
def failure(): #サーバーがタイムアウトした時間と復旧した時間を調べる
    ...
    for line in log_data:
        network_failure(line) #ネットワークの故障を確認する
...
def failure_period():
    ...
    #サブネットの故障
    for line in server_network_failure:
        #もしサーバーがすでに復旧している場合
        if line[8] == 'failure3':
            #print(line[6],'サーバーは現在まで故障しています')
            server_network_failure_log.append([[line[0],time_now.year],[line[1],time_now.month],[line[2],time_now.day],[line[3],time_now.hour],[line[4],time_now.minute],[line[5],time_now.second],line[6],[line[7],time_now.microsecond / 1000],'failure3'])
    for line in server_network_failure_log:
        period_calculation(line)
...
def period_calculation(server):
    ...
    if server[8] == 'restoration':
        ...
        print('でした。')
        print()
    elif server[8] == 'failure3':
        print('サブネット',server[6],'のネットワークは現在も故障しています。')
        ...
        print("経過しています。")
        print()
    elif server[8] == 'restoration3':
        print('サブネット',server[6],'のネットワークは復旧済です。')
        print(failure_start[0],'年',end=' ')
        print(failure_start[1],'月',end=' ')
        print(failure_start[2],'日',end=' ')
        print(failure_start[3],'時',end=' ')
...
def network_failure(server): #各サブネット毎にネットワークの故障期間を出力
    if server[7] == '-':
        duplicate = False
        for line in server_network_failure:
            if line[6] == server[6].split('/')[1]: 
                #print(line[6],'サブネットのネットワークはタイムアウト継続です')
                line[9] += 1
                if line[9] >= N:
                    #print(line[6],'サブネットのネットワークは故障しています')
                    line[8] = 'failure3'
                duplicate = True
        if duplicate == False:
            #print('新たに',server[6].split('/')[1],'サブネットのネットワークがタイムアウトしました')
            server_network_failure.append([server[0],server[1],server[2],server[3],server[4],server[5],server[6].split('/')[1],server[7],'timeout',1])
            if server_network_failure[-1][9] >= N:
                    #print('新たに',server[6].split('/')[1],'サブネットのネットワークは故障しました')
                    server_network_failure[-1][8] = 'failure3'
        duplicate = False
        ...
        for line in server_network_failure:
            if line[6] == server[6].split('/')[1]:
                if line[8] == 'failure3':
                    #print(server[6].split('/')[1],'サブネットのネットワークが復旧しました')
                    server_network_failure_log.append([[line[0],server[0]],[line[1],server[1]],[line[2],server[2]],[line[3],server[3]],[line[4],server[4]],[line[5],server[5]],line[6],[line[7],int(server[7])],'restoration3'])
                else:
                    pass
                    #print(line[6],'サーバーは故障とみなしませんでした')
                server_network_failure.remove(line)
